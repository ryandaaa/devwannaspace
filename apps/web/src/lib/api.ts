...
const API_BASE = localStorage.getItem('devwannaspace_custom_api_url') || import.meta.env.VITE_API_URL || 'https://api.ryanda-valents3649.workers.dev/api';
...
async function smartSync<T extends { id: string }>(
  ...
  const updated = newItems.filter(newVal => {
     ...
     // For performance, compare updatedAt timestamps instead of JSON.stringify on potentially huge content
     if ((newVal as any).updatedAt && (oldVal as any).updatedAt) {
       return new Date((newVal as any).updatedAt).getTime() !== new Date((oldVal as any).updatedAt).getTime();
     }
