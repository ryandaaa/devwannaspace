...
export const IssuesView = forwardRef<IssuesViewRef, IssuesViewProps>(({ 
  ...
  issues, projects, onCreateIssue, onUpdateIssue, onDeleteIssue, onCreateProject, lockedProjectId
}, ref) => {
  ...
  const [newProjId, setNewProjId] = useState(lockedProjectId || (projects.length > 0 ? projects[0].id : ''));
  ...
  const handleOpenCreate = (initialStatus?: IssueStatus, initialDate?: string) => {
    setNewTitle('');
    setNewDesc('');
    setNewProjId(lockedProjectId || (projects.length > 0 ? projects[0].id : ''));
