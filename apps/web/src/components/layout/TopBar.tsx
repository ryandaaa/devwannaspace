...
import {
  ...
  Menu, Settings, Download, Sun, Moon, Clock, Maximize
} from 'lucide-react';
        ...
        {currentPage && (
          ...
          {currentPage && (
            <div style={{ position: 'relative' }} ref={projectMenuRef}>
              <button
                onClick={() => { setProjectMenuOpen(!projectMenuOpen); setProjectSearch(''); }}
                style={{
              ...
              {projectMenuOpen && (
                <div style={{
                  ...
                  width: 220, backgroundColor: 'var(--surface-1)',
                  ...
                  <div style={{ padding: '4px 12px', fontSize: 11, fontWeight: 600, color: 'var(--ink-subtle)', textTransform: 'uppercase' }}>
                    Select Project
                  </div>
                  ...
                  {projects.filter(p => p.name.toLowerCase().includes(projectSearch.toLowerCase())).map(p => (
                    <button
                      key={p.id}
                      onClick={() => { onUpdatePage(currentPage.id, { projectId: p.id }); setProjectMenuOpen(false); setProjectSearch(''); }}
                      style={{
                        width: '100%', padding: '8px 12px', textAlign: 'left',
                        background: 'none', border: 'none', cursor: 'pointer',
          ...
          {/* Zen Mode Toggle */}
          <IconBtn onClick={onToggleZenMode} title="Enter Zen Mode (Alt+Z)">
            <Maximize size={16} />
          </IconBtn>
